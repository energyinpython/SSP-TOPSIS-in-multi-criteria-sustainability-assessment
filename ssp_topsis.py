...
class SSP_TOPSIS(MCDA_method):
    def __init__(self, normalization_method = norms.minmax_normalization, distance_metric = dists.euclidean):
        """
        Create the SSP_TOPSIS method object and select normalization method `normalization_method` and
    ...
    def __call__(self, matrix, weights, types, s_coeff = 0):
        ...
        Examples
        ---------
        >>> ssp_topsis = SSP_TOPSIS(normalization_method = minmax_normalization, distance_metric = euclidean)
        >>> pref = ssp_topsis(matrix, weights, types, s_coeff)
        >>> rank = rank_preferences(pref, reverse = True)
        """
        
        SSP_TOPSIS._verify_input_data(matrix, weights, types)
        return SSP_TOPSIS._ssp_topsis(self, matrix, weights, types, self.normalization_method, self.distance_metric, s_coeff)
    ...
    def _equalization(self, matrix, types, s_coeff):
        ...
        for j, i in itertools.product(range(matrix.shape[1]), range(matrix.shape[0])):
            # procedure for profit criteria
            if types[j] == 1:
                if mad[i, j] < 0:
                    mad[i, j] = 0
            # procedure for cost criteria
            elif types[j] == -1:
                if mad[i, j] > 0:
                    mad[i, j] = 0
    ...
    @staticmethod
    def _ssp_topsis(self, matrix, weights, types, normalization_method, distance_metric, s_coeff):
